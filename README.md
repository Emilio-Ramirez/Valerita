...
### Overview
The website now features an **automated content synchronization system** that syncs `CONTENT.md` → `js/i18n.js` automatically when committing changes.

### How It Works
1. Content editor updates `CONTENT.md` (Spanish content only)
2. Editor commits changes: `git add CONTENT.md && git commit -m "Update content"`
3. **Git pre-commit hook** automatically:
   - Detects CONTENT.md changes
   - Runs sync script (`scripts/sync-content.mjs`)
   - Updates `js/i18n.js` with new Spanish content
   - Preserves existing English translations
   - Stages updated `i18n.js` for commit
4. Changes are ready to push and deploy
...
### Files
- **`CONTENT.md`** - Content editing file (Spanish only)
- **`scripts/sync-content.mjs`** - Sync automation script
- **`.husky/pre-commit`** - Git hook that triggers sync
- **`package.json`** - Node.js configuration
- **`js/i18n.js`** - Translation file (auto-updated)
...
### For Content Editors
👉 **Edit `CONTENT.md` only** - The system handles everything else automatically!

See [CONTENT.md](CONTENT.md) for detailed editing instructions.

### For Developers

**Installing Dependencies:**
```bash
npm install  # Installs husky for git hooks
```

**Testing the Sync:**
```bash
# Make a test change to CONTENT.md
# Then commit to trigger the hook
git add CONTENT.md
...
```bash
git commit --no-verify -m "Skip sync"
```

### Current Limitations
- ⚠️ Sync script changes quote style (single → double quotes)
- ⚠️ English translations preserved but not auto-generated (manual for now)
- 💡 Future: Add AI-powered translation integration
